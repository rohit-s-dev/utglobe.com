...
if (mapWrap.length) {
	...
	$('.sub-dropdown-menu > ul').hide();
	$('.sub-dropdown-menu').click(function(){

		$('.sub-dropdown-menu > ul').slideToggle();
...
$('.s_m').on('click', function(){
	$('.feed-box').toggle('f_show');
});


$('#f_sub').on('click', function(e){
	e.preventDefault();
	...
	if (name === '' || mob === '') {
		alert('Please check your name or mobile number');
	} else {
		$.ajax({
			type: "post",
			url: "assets/ajax/main.php",
			data: {name: name, email:email, mob:mob},
			beforeSend: function() {
				$('#f_sub').html('Please Wait....');
				$('#f_sub').attr('disabled', 'disabled');
			},
			success: function (data) {
				if (!data.error) {
					$('.m').html(data);
					$('#f_sub').html('Submitted');
					$('#f_sub').attr('disabled', 'disabled');
					$('.p_right_box').html('<h5>Thank You ! You will be redirected to feedback page soon</h5>');
					$('.m_t').html('<p style="color:white; text-align:center;">Thank You ! You will be redirected to feedback page soon</p>');
					$('#name').val('');
					$('#email').val('');
					$('#m_numb').val('');
...
$('#s_submit').on('click', function(e){
	...
	var category = $('#s_category').val();
		...
		$.ajax({
			...
			data: {name:name, email:email, msg:msg, category:category},
			beforeSend: function () {
				$('#s_submit').val('Please Wait...');
				$('#s_submit').attr('disabled', 'disabled');
			},
			success: function (data) {
				if (!data.error) {
					$('.m_t').html(data);
					$('#s_submit').val('Submitted');
...
$('#contact_f').on('submit' , function(e){
    e.preventDefault();
